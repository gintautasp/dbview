	...
	class db_mysql_user1 {
		...
		function __construct ( $name_db, $name_user_db = 'root', $password_db = '', $log_queries = false, $name_server_db = 'localhost', $persistent = false ) {
			...
			if ( !empty ( $name_db ) ) { 
				...
				else 	$this->ercl_db = mysql_connect ( $name_server_db, $name_user_db, $password_db );	
				...
				if ( ! $this->ercl_db ) {
					...
					mysql_set_charset ( 'utf8', $this->ercl_db );
		...
		public function ready_db ( $name_db = false ) { 
			...
			if ( ! mysql_select_db ( $this->name_db, $this->ercl_db ) ) $this->take_mysql_error ( 'USE `' . $this->name_db . '`' );
		...
		public function take_mysql_error ( $query, $debug_backtrace_level = 'a' ) {
			...
			$this->inf_db_errors[] = array ( 
								'mysql_errno' =>  mysql_errno ( $this->ercl_db )
								, 'mysql_error' => mysql_error ( $this->ercl_db )
		...
		public function perform ( $query, $take = 'x', $debug_backtrace_level = 2, $log = false ) {
			...
			if ( $this->flag_log_queries || $log ) { 
				...
				$log_query->loc = get_debug_backtrace ( $debug_backtrace_level, debug_backtrace() );
			...
			if ( ! empty ( $query ) && ( $this->status == 'ready' ) ) {
				...
				if  ( ! ( $rsQuery = mysql_query ( $query, $this->ercl_db ) ) ) {
					...
					if ( $take == 'affected_rows' ) $this->last_afected_rows = mysql_affected_rows ( $this->ercl_db );
					
					if ( $take == 'last_insert_id' ) $this->last_insert_id =  mysql_insert_id ( $this->ercl_db );
					
					if ( $take == 'mysql_num_rows' ) $this->mysql_num_rows = mysql_num_rows ( $rsQuery );
				}
